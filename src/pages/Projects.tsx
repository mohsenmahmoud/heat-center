  ...
  return (
    <>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {projectTypes.map((p, i) => {
              ...
              return (
                <div
                  key={p.title}
                  className="group relative flex flex-col overflow-hidden rounded-3xl border border-ink-950/8 bg-white shadow-sm"
                >
                  ...
                  <div className="relative flex flex-col gap-4 p-8">
                    ...
                    <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-ember-500/10 text-ember-500 ring-1 ring-inset ring-ember-500/20">
        ...
        <Container className="relative">
          ...
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {caseStudies.map((project, i) => (
              <div
                key={i}
                className="flex flex-col overflow-hidden rounded-3xl border border-ink-950/8 bg-white shadow-sm"
              >
                ...
                <div className="flex flex-1 flex-col gap-2 p-6">
                  <span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-ember-500/10 px-3 py-1 text-[11px] font-bold text-ember-600 ring-1 ring-inset ring-ember-500/20">
                    {project.projectType}
                  </span>
                  <h3 className="font-display text-base font-bold text-ink-950">{project.title}</h3>
                  <p className="text-sm leading-relaxed text-ink-500">{project.scope}</p>
      ...
      <section className="relative bg-white py-24">
        <Container>
          <div className="grid grid-cols-2 gap-8 rounded-3xl border border-ink-950/8 bg-ink-50 px-8 py-10 sm:grid-cols-4">
            {stats.map((s) => (
              <div key={s.label} className="flex flex-col items-center gap-1 text-center">
                <span className="font-display text-3xl font-extrabold text-ink-950 sm:text-4xl">
                  {s.value}
          ...
          <div className="mt-14 flex flex-col items-center gap-5 text-center">
            <h2 className="max-w-xl text-balance font-display text-2xl font-extrabold text-ink-950 sm:text-3xl">
